...
# check files
BASEDIR=$(dirname "$0")
FILE_LIST=("$BASEDIR/checker" "$BASEDIR/pd24_results.list" $1 $2 $3)
for FILE_NAME in ${FILE_LIST[@]}; do
    if [ ! -f $FILE_NAME ]; then
        echo "[Eval] error: Please check file '$FILE_NAME' exist."
        echo "(ps. checker and pd24_result.list should be in the same dir with evaluator.sh)"
        exit 1
    fi
done
# check alpha
